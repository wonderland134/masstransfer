    ...
    def calc(self):
        ...
        for i in range(len(t_array)):
            for j in range(len(x_array)):
                ...
                y_array[i][j] = sp.erfc(z) * self.C0
                y_array[i][j] = (21250-769.23)/(50-0)*y_array[i][j]+769.23        #change to intensity
...
if __name__ == '__main__':
    test = emulate(2.25*10**-6, 'Emulated by erfc, BMP-2 in 6% collaen')
    test.calc()
    test.plotting()