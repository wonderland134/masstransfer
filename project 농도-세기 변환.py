...
D = 1.25*10**-6 		    #cm^2/s, Bovine alpha-Chymotrypsin through 6% collagen
L = 4 					#cm
...
for i in range(len(t_array)):
    ...
    if flag != -1:
        ...
        I_array = result[0]*np.log(C_array-result[1])+result[2]
        np.save('{}min_sample.npy'.format(t), I_array)
        plt.plot((x_array[xi_1:xi_2]-x_array[xi_1])*10000, I_array[xi_1:xi_2], label = 't = {} min'.format(t))
        flag = -1
